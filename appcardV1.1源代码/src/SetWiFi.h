...
const char* AP_NAME = "appcard";//Web配网模式下的AP-wifi名字

//暂时存储wifi账号密码
char sta_ssid[32] = {0};
char sta_password[64] = {0};
char sta_citycode[32] = {0};
char sta_biliUID[64] = {0};
	...
	<head>
		...
		<style type="text/css">
			* { margin: 0; padding: 0; }
			 ...
			 .dowebok { position: absolute; left: 50%; top: 30%; width: 380px; height: 750px; margin: -200px 0 0 -200px; border: 3px solid #fff; border-radius: 10px; overflow: hidden;}
	...
	<body>
		<div class="dowebok">
			...
			<form style='text-align: center;padding-top: 20px' name='input' action='/' method='POST'>	 
				 ...
				 <div class="form-item">
				 	<input id="bilicode" type="text" name='biliUID' autocomplete="off" placeholder="B站用户UID,留空则用系统预留">
				 </div>
				 <div class="form-item">
				 	<input id="nzhhh" type="text" name='nzhhh' autocomplete="off" placeholder="闹钟小时,留空则为0">
				 </div>
				 
                 <div class="form-item">
				 	<input id="nzmmm" type="text" name='nzmmm' autocomplete="off" placeholder="闹钟分钟,留空则为0">
				 </div>
				 
			      <div class="form-item">
				 	<input id="nzsss" type="text" name='nzsss' autocomplete="off" placeholder="闹钟秒,留空则为0">
				 </div>
				...
				<div class="form-item">
					<div class="user_text">
						<br>
						<p><h5>1、CityCode文件夹的cityCode.js中，AREAID对应的参数即是城市代码</h5>
			               <h5>2、B站用户UID是阿拉伯数字组成，非用户名或者昵称</h5>
						   <h5>3、闹钟小时是24小时制</h5> 							
						</p>
...
void handleRootPost() {//Post回调函数
  ...
  if (server.hasArg("biliUID")) {
    Serial.print("got biliUID:");
    strcpy(sta_biliUID, server.arg("biliUID").c_str());
    Serial.println(sta_biliUID);
  } else {
    Serial.println("error, not found biliUID");
    server.send(200, "text/html", "<meta charset='UTF-8'>提示：请输入B站UID");
    return;
  }
  

  if (server.hasArg("nzhhh")) {
    Serial.print("got nzhhh:");
    strcpy(sta_nzhhh, server.arg("nzhhh").c_str());
    Serial.println(sta_nzhhh);
  } else {
    Serial.println("error, not found nzhhh");
    server.send(200, "text/html", "<meta charset='UTF-8'>提示：请输入小时");
    return;
  }
  
  
  if (server.hasArg("nzmmm")) {
    Serial.print("got nzmmm:");
    strcpy(sta_nzmmm, server.arg("nzmmm").c_str());
    Serial.println(sta_nzmmm);
  } else {
    Serial.println("error, not found nzmmm");
    server.send(200, "text/html", "<meta charset='UTF-8'>提示：请输入分钟");
    return;
  }
  
  
  if (server.hasArg("nzsss")) {
    Serial.print("got nzsss:");
    strcpy(sta_nzsss, server.arg("nzsss").c_str());
    Serial.println(sta_nzsss);
  } else {
    Serial.println("error, not found biliUID");
    server.send(200, "text/html", "<meta charset='UTF-8'>提示：请输入秒");
    return;
  }
  ...
  preferences.putString( "biliUID", sta_biliUID);

 
 preferences.putString( "nzhhh", sta_nzhhh);
 preferences.putString( "nzmmm", sta_nzmmm);
 preferences.putString( "nzsss", sta_nzsss);
 
 
  preferences.end();

  server.send(200, "text/html", "<meta charset='UTF-8'><h1>保存成功，appcard重启中...</h1>");//返回保存成功页面
  delay(2000);
  //连接wifi
  //connectNewWifi();
